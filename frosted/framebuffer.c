...
int framebuffer_setcmap(const uint32_t *ptr)
{
    if (!fb[0] || !fb[0]->fbops || !fb[0]->fbops->fb_setcmap)
        return -ENODEV;
...
{
    if (!fb[0] || !fb[0]->fbops || !fb[0]->fbops->fb_update)
        return -ENOSYS;
    return fb[0]->fbops->fb_update(fb[0], x, y, w, h);
}
