...
void * f_malloc(int flags, uint32_t size);
void *f_malloc_pid(int flags, size_t size, uint16_t owner);
void * f_calloc(int flags, uint32_t num, uint32_t size);
void* f_realloc(int flags, void* ptr, uint32_t size);
void f_free(void * ptr);

/* Free up heap of a specific pid */
void f_free_all(uint16_t pid);
