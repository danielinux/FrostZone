...
#define STM32_UART_PRESC_DEFAULT 2U
...
static uint32_t stm32_uart_bus_clock(uint32_t base)
{
...
static uint32_t stm32_uart_presc_div(uint32_t presc)
{
    static const uint16_t presc_table[] = {
        1U, 2U, 4U, 6U, 8U, 10U, 12U, 16U, 32U, 64U, 128U, 256U,
    };
    uint32_t idx = presc & 0xFU;
    uint32_t table_len = (sizeof(presc_table) / sizeof(presc_table[0]));

    if (idx >= table_len)
        return presc_table[table_len - 1U];
    return presc_table[idx];
}

static uint32_t stm32_uart_brr(uint32_t base, uint32_t baudrate, uint32_t presc)
{
    uint32_t clock = stm32_uart_bus_clock(base);
    uint32_t div = stm32_uart_presc_div(presc);

    if (!baudrate || clock == 0U || div == 0U)
        return 0;
...
int uart_create(const struct uart_config *cfg)
{
    ...
    {
        uint32_t presc = STM32_UART_PRESC_DEFAULT;
        uint32_t baudrate = cfg->baudrate ? cfg->baudrate : 115200U;

        port->regs->PRESC = presc;
        port->regs->BRR = (uint16_t)stm32_uart_brr(cfg->base, baudrate, presc);
    }
