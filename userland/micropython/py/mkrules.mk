...
# Determine if $(TOP) is the root of a git checkout. Some downstream trees vendor
# MicroPython and do not expose its .git directory, which breaks submodule calls.
MICROPY_GIT_TOP := $(shell cd $(TOP) 2>/dev/null && git rev-parse --show-toplevel 2>/dev/null)
MICROPY_IS_GIT_ROOT := $(strip $(filter $(abspath $(TOP)),$(MICROPY_GIT_TOP)))
...
ifeq ($(MICROPY_IS_GIT_ROOT),)
	$(ECHO) "Skipping git submodule update (MicroPython tree is vendored without its own .git directory)."
else
